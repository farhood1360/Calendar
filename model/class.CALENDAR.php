...
class Calendar {
    ...
    public function render(){
        ...
        $this->content .= "<a href=\"{$_SERVER['PHP_SELF']}?year={$this->nextYearNav}\" id='nextYear'>
        ...
        while($this->dayNum <= $this->daysCount){ 
            if($this->dayNum == date('j', $this->christmas) && $this->month == 12){
                if($this->dayNum == $this->day && $this->month == date('m') && $this->year == date('Y')){
                    $this->content .= "<td class='today'>{$this->dayNum}<br/><b><i>Today</i></b><br/>Christmas</td>";
                    $this->dayNum++;
                    $this->day_count++;
                }else{
                ...
                if($this->dayNum == $this->day && $this->month == date('m') && $this->year == date('Y')){
                    $this->content .= "<td class='today'>{$this->dayNum}<br/><b><i>Today</i></b><br/>New Year</td>";
                    $this->dayNum++;
                    $this->day_count++;
                }else{
                ...
                if($this->dayNum == $this->day && $this->month == date('m') && $this->year == date('Y')){
                    $this->content .= "<td class='today'>{$this->dayNum}<br/><b><i>Today</i></b><br/>Indep</td>";
                    $this->dayNum++;
                    $this->day_count++;
                }else{
                    $this->content .= "<td class='event'> $this->dayNum <br/><b>Indep</b></td>";
                    $this->dayNum++;
                    $this->day_count++;
                }
            }else if($this->dayNum == date('j', $this->halloween) && $this->month == 10){
                if($this->dayNum == $this->day && $this->month == date('m') && $this->year == date('Y')){
                    $this->content .= "<td class='today'>{$this->dayNum}<br/><b><i>Today</i></b><br/>Halloween</td>";
                    $this->dayNum++;
                    $this->day_count++;
                }else{
